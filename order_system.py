def place_order(menu):
    ...
    while True:
        ...
        # Extract the food category and the options for each category
        for food_category, options in menu.items():

            # Loop through the options for each food category
            # Extract the meal and the price for each option
            for meal, price in options.items():

                # Print the menu item number, food category, meal, and price
                # Only if you used different variable names
                # Update the variable names in the following function
                print_menu_line(i, food_category, meal, price)

                # Update the menu selection number
                i += 1

        # Ask customer to input menu item number in variable
        menu_selection = input("Please enter the number of item(s) you would like to order: ")
...
def update_order(order, menu_selection, menu_items):
    ...
    if menu_selection.isdigit():
        ...
        if menu_selection in menu_items:
            ...
            input_quantity = input(f"How many {item_name} would you like to order? ")
        ...
        # tell the customer that their input isn't valid
            ...
            print(f"Your selection is invalid: {menu_selection}")
...
def print_itemized_receipt(receipt):
    ...
    for item in receipt:
        ...
        print_receipt_line(item_name, item_price, quantity)
